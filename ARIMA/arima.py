...
def load_and_preprocess_data(filepath, chunksize=100000):
    ...
    iterator = pd.read_csv(filepath, header=None, usecols=[0, 1, 2, 3, 4, 5, 7], names=['CNSMR_NO', 'LEGALDONG_CD', 'MESURE_YEAR', 'MESURE_MT', 'MESURE_DTTM', 'Hour', 'SGPowerUsage'], chunksize=chunksize)
...
def auto_select_order_and_predict(df, n_periods):
    ...
    for (leg_cd, cns_no), group in tqdm(df.groupby(['LEGALDONG_CD', 'CNSMR_NO']), desc="Model Fitting & Prediction"):
        ...
        auto_model = auto_arima(series, seasonal=True, m=12, trace=False, error_action='ignore', suppress_warnings=True, stepwise=True)
